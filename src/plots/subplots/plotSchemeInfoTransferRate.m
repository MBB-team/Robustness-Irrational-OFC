function [] = plotSchemeInfoTransferRate(ax)
% Code for figure 5c.
%
% INPUTS ------------------------------------------------------------------
...
% Compte output entropy for a given input distribution
unit_inputs = linspace(-10, 10, 1000);
input_distrib = normpdf(unit_inputs, 2.5, 2);
activation_function = sigANN(unit_inputs)';
exp_unit_inputs = exp(- unit_inputs);
deriv_activation = exp_unit_inputs ./ ((1 + exp_unit_inputs) .^ 2);

% Draw output entropy
fill(ax, unit_inputs(1) + [deriv_activation, zeros(size(deriv_activation))], ...
    [linspace(0, 1, 1000), fliplr(linspace(0, 1, 1000))], "k", ...
    EdgeColor="none", ...
    FaceAlpha=0.2, ...
    DisplayName="Output entropy");
