...
%     "prop_none_Franck", "prop_none_Miles"], "rational_last");
...
for i_config = 9:10
    ...
    for i_cell = 1:length(all_cell_type)
        ...
        % Plot monkey data
        x_monkey = i_cell + (plot_options.distrib_config_x_shift + plot_options.distrib_density_width) * [-1, 1];
        for monkey = ["Franck", "Miles"]
            plot(ax, x_monkey, ...
                MonkeyData.(monkey).("prop_" + all_cell_type(i_cell)) * ones(1, 2), ...
                Color="k", ...
...
ylim(ax, [0, 100]);
xticks(ax, 1:4);
xticklabels(ax, ["Offer value", "Chosen value", "Chosen offer", "None"]);
