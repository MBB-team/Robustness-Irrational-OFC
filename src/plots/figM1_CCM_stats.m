...
% 
% c, Each cell in the CCM matrix shows the correlation across neurons for a
% given pair of regression coefficients (neurons pooled across monkeys).
% The upper half of the matrix shows the results computed on option trials
% (where the two first cues characterize the same option), while the lower
% half corresponds to attribute trials (where the two first cues
% characterize the same attribute, but different options). Asterisks
...
plotCCM(nexttile(3, [2, 2]), ...
    NeuralGeometry.OFC.both.CCM_option, NeuralGeometry.OFC.both.CCM_attribute, ...
...
writePanelLetter(nexttile(3, [2, 2]), "c", -0.4, 0.3);
...
saveas(f, fullfile(getPath("Figures"), "figM1.pdf"));
