...
while ~stop
    ...
    if options.updateHP
        ...
        suffStat = updateSuffStatHistory(suffStat, posterior, options);
    ...
    if isfield(options, 'store_history') && options.store_history
        try
            suffStat.i_end_GnLoop = [suffStat.i_end_GnLoop, ...
                size(suffStat.params_history, 2)];
        catch
            suffStat.i_end_GnLoop = size(suffStat.params_history, 2);
        end
    end
